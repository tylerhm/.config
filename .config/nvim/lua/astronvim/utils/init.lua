...
function M.reload(quiet)
  ...
  local core_modules = { "tylerhm.bootstrap", "tylerhm.options", "tylerhm.mappings" }
  ...
  if not quiet then -- if not quiet, then notify of result
    if success then
      M.notify("Successfully reloaded", vim.log.levels.INFO)
    else
      M.notify("Error reloading...", vim.log.levels.ERROR)
    end
  end
